...
namespace MoreMountains.TopDownEngine
{
	...
	public class DamageOnTouch : MMMonoBehaviour
	{
		...
		[Header("Ability Mind Control")]
		/// the type of knockback to apply when causing damage
		[Tooltip("Apakah akan ada mind control atau tidak")]
		public bool MindControls = false;
		[Header("Knockback")]
		/// the type of knockback to apply when causing damage
		...
		protected virtual void OnCollideWithDamageable(Health health)
		{
			...
			if (health.CanTakeDamageThisFrame())
			{
				...
				// Setelah menerapkan damage
				CharacterMindControl mindControl = health.GetComponent<CharacterMindControl>();
				if (mindControl != null && MindControls)
				{
					mindControl.ApplyMindControl();
				}
